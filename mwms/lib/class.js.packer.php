		...
		if (!empty($all)) {
			...
			do {
				...
				if (isset($protected[$word]) /*!= null*/) {
					$_sorted[$protected[$word]] = mb_substr($word, 1);
			...
			do {
				if (!isset($_sorted[$i]))
					$_sorted[$i] = mb_substr($unsorted[$j++], 1);
...
class ParseMaster {
		...
		if (is_string($replacement)) {
			...
			if (preg_match($this->SUB_REPLACE, $replacement)) {
				...
				if (preg_match($this->INDEXED, $replacement)) {
					...
					$replacement = (int)(mb_substr($replacement, 1)) - 1;
	...
	public function exec($string) {
		...
		foreach ($this->_patterns as $reg) {
			$regexp .= '(' . mb_substr($reg[self::EXPRESSION], 1, -1) . ')|';
		}
		$regexp = mb_substr($regexp, 0, -1) . '/';
		...
		while (isset($this->_patterns[$j])) {
			...
			if (isset($arguments[$i]) && ($arguments[$i] != '')) {
				...
				$delete = ($this->escapeChar == '' ||
				           mb_strpos($arguments[$i], $this->escapeChar) === false)
				        ? '' : "\x01" . $arguments[$i] . "\x01";
	...
	private function _replace_name($match, $offset){
		$length = mb_strlen($match[$offset + 2]);
		$start = $length - max($length - mb_strlen($match[$offset + 3]), 0);
		return mb_substr($match[$offset + 1], $start, $length) . $match[$offset + 4];
	}
