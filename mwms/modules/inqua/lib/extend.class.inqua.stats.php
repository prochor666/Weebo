<?php
class InquaStats extends WeeboInqua{
...
public function getInquirySet($id_asset){
	
	$o = array();
	
	if($id_asset>0){
		
		$q = "SELECT * FROM "._SQLPREFIX_."_inquiry_assets WHERE id_asset = ".(int)$id_asset." ORDER BY id_asset";
		//$qq = Db::result($q);
	...
	$csvRow .= '" ";" ";" ";" "'."\r\n";
	$csvRow .= '" ";" ";" ";"'.$this->lng['inquiry_stat_ctr_summary'].'"'."\r\n";
	$csvRow .= '"'.$this->lng['inquiry_stat_summary'].'";'.$dayImpressionsAll.';'.$dayClicksAll.';'.str_replace('.', ',', (string)$rowCTRAll).''."\r\n";
	
	$o['title'] = $this->lng['inquiry_stat_main_label'].': '.$d['title'];
	$o['label_click'] = $this->lng['inquiry_stat_click_label'];
	$o['label_impress'] = $this->lng['inquiry_stat_impress_label'];
	$o['label_date'] = $this->lng['inquiry_stat_date_label'];
...
protected function getAssetGrid($id_asset, $from, $to){
	...
	foreach($x as $item){
		...
		$qi = "SELECT * FROM "._SQLPREFIX_."_inquiry_asset_stats WHERE id_asset = ".$id_asset." AND action_time >= ".$item_store_time." AND action_time < ".($item_store_time + 86400)." AND action_type = 2 ";
		$qc = "SELECT * FROM "._SQLPREFIX_."_inquiry_asset_stats WHERE id_asset = ".$id_asset." AND action_time >= ".$item_store_time." AND action_time < ".($item_store_time + 86400)." AND action_type = 1 ";
		//$di = Db::result($qi);
		//$dc = Db::result($qc);
